...
for(let i = 0; i < posts.length; i++) {
    ...
    postHTML.querySelector(".js-like-button").addEventListener("click", function(e) {
        ...
        if(!postLiked.includes(id)) {
            ...
            // 3Bonus. Al click su un pulsante "Mi Piace" di un post, se abbiamo già cliccato dobbiamo decrementare il contatore e cambiare il colore del bottone
            counterHTML.innerHTML--;
...
// 1Bonus. Formattare le date in formato italiano (gg/mm/aaaa)
function italianDate(date) {
    return date.split("-").reverse().join("/");
}
// 2Bonus. Gestire l'assenza dell'immagine profilo con un elemento di fallback che contiene le iniziali dell'utente (es. Luca Formicola > LF).
